...
export function update (type = false, bol = false) {
  ...
  keys.forEach((item, index) => {
    datastr = index == keys.length - 1 ?
      `${datastr}${item}=${checkOptType(newopt[item], item, type, bol)}` : `${datastr}${item}=${checkOptType(newopt[item], item, type, bol)},`
  })
  result = this.sqlObj.where ?
    `UPDATE ${this.sqlObj.table} SET ${datastr} WHERE ${this.sqlObj.where}` : `UPDATE ${this.sqlObj.table} SET ${datastr}`
...
export function delet (type = false) {
  let result = this.sqlObj.where ?
    `DELETE FROM ${this.sqlObj.table} WHERE ${this.sqlObj.where}` : `DELETE FROM ${this.sqlObj.table}`
  const sqlStr = result.replace(/'/g, '\'').replace(/`/g, '\'')
  if (type) {
    this.sqlObj.sqlStr = sqlStr
    return this
